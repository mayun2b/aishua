...
- 统计粒度：按 `user_id + tag_id` 维护 `total_count`、`correct_count`、`wrong_count`、`correct_rate`、`mastery_level`。
- 小样本保护：当该知识点作答数 `< 20` 时，按正确率分级但最高只到 `2` 级。
- 稳定分级：当作答数 `>= 20` 时，按正确率直接分级。
- 默认阈值：
- `>= 90%` -> `3` 级
- `>= 75%` -> `2` 级
- `>= 60%` -> `1` 级
- `< 60%` -> `0` 级
- 可配置项：`application.yml` 下 `practice.mastery.*`
